...
int RegressionCompare::add(std::vector<float> &required_value, std::vector<float> &output_value)
{
    if (is_valid(required_value) == false)
    {
        this->nan_error = true;
        return -1;
    }

    if (is_valid(output_value) == false)
    {
        this->nan_error = true;
        return -2;
    }

    if (required_value.size() != output_value.size())
    {
        this->nan_error = true;
        return -3;
    }
...
void RegressionCompare::compute(int fixed_bars_count)
{
    if (count == 0)
        return;
...
bool RegressionCompare::is_nan_error()
{
...
bool RegressionCompare::is_valid(std::vector<float> &v)
{
