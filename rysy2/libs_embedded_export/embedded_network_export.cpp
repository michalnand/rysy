...
int EmbeddedNetworkExport::process(std::string export_path, std::string network_prefix)
{
  ...
  for (unsigned int i = 0; i < json_parameters["layers"].size(); i++)
  {
    ...
    if ( (layer["type"].asString() == "dense fc") || (layer["type"].asString() == "dense_fc") )
    {
      ...
      output_shape.w = 1;
      output_shape.h = 1;
      output_shape.d = shape.d + input_shape.w*input_shape.h*input_shape.d;
    ...
    */
    ...
    if ((layer["type"].asString() == "max pooling")||(layer["type"].asString() == "max_pooling"))
    {
      ...
      output_shape.w = input_shape.w/shape.w;
      output_shape.h = input_shape.h/shape.h;
      ...
      network_config_cpp+= "\t\tlayers[" + std::to_string(layer_idx)+"] = ";
      network_config_cpp+= "new NetMaxPoolingLayer(" + s_shape + "," + s_input_shape + "," + s_output_shape + ");";
      network_config_cpp+= "\n";
