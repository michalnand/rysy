...
__global__
void cuda_relu_forward_kernel(float *output, float *input, unsigned int size)
{
...
void relu_layer_forward(  Tensor &output, Tensor &input)
{
  unsigned int size = output.size();
    ...
    cpu_relu_forward_kernel(output.v, input.v, size);
...
__host__
void cpu_relu_backward_kernel(float *error_back, float *error, float *output, unsigned int size)
{
  for (unsigned int idx = 0; idx < size; idx++)
  {
    if (output[idx] > 0.0)
...
__global__
void cuda_relu_backward_kernel(float *error_back, float *error, float *output, unsigned int size)
{
  unsigned int idx = threadIdx.x + blockIdx.x*blockDim.x;
...
void relu_layer_backward( Tensor &error_back, Tensor &output, Tensor &error)
{
  unsigned int size = output.size();
    ...
    cpu_relu_backward_kernel(error_back.v, error.v, output.v, size);
