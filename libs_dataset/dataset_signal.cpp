...
DatasetSignal::DatasetSignal(unsigned int classes_count, unsigned int length, float noise_level)
{
  width      = length;
  height     = 1;
  channels   = 1;
...
void DatasetSignal::create(unsigned int items_count, unsigned int length, float testing_ratio, float noise_level)
{
    ...
    for (unsigned int i = 0; i < items_count; i++)
    {
        ...
        item.input = signal(frequency, length, noise_level);
...
std::vector<float> DatasetSignal::signal(float frequency, unsigned int length, float noise_level)
{
