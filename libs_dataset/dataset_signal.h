...
{
    public:
        DatasetSignal(unsigned int classes_count, unsigned int length = 128, float noise_level = 0.1);
        virtual ~DatasetSignal();

    private:
        void create(unsigned int items_count, unsigned int length, float testing_ratio, float noise_level);
        std::vector<float> signal(float frequency, unsigned int length, float noise_level);
